...
## Условие
Разместить **k центров** на плоскости так, чтобы минимизировать суммарное расстояние от каждой из **n точек-клиентов** до ближайшего центра.

**Алгоритм**: имитация отжига: случайное смещение центров, принятие решения по критерию Метрополиса.
...
- ✅ если `new_cost < current_cost`, принимаем;
- ✅ иначе принимаем с вероятностью `exp((current_cost - new_cost)/temp)`

### Обновление centers и current_cost при принятии

## Примечания:
- Функция `totalDistance` считает сумму расстояний от каждого клиента до ближайшего центра
...
## **Ответ на контрольные вопросы 20 вариант** 



## 1. **Логистика и транспорт: Задача маршрутизации (Vehicle Routing Problem)**

**Проблема:** Построение оптимальных маршрутов для доставки товаров клиентам.

**Пример:** 
- **Google Maps, Яндекс.Карты** — построение маршрутов
- **Amazon, СберМаркет** — планирование маршрутов курьеров
- **Почта России** — оптимизация почтовых маршрутов
...
## 2. **Телекоммуникации: Размещение вышек сотовой связи**

**Проблема:** Размещение минимального количества вышек для покрытия территории с заданным качеством сигнала.

**Пример:**
- **МТС, Билайн, МегаФон** — планирование сетевой инфраструктуры
- **Wi-Fi провайдеры** — размещение точек доступа

**Используемые алгоритмы:**
- Имитация отжига (как в моей задаче)
- Генетические алгоритмы
- Жадное добавление точек

## 3. **Производство: Задача коммивояжера (TSP)**

**Проблема:** Найти кратчайший маршрут через все города с возвратом в исходный.

**Пример:**
- **Производственные предприятия** — маршруты для обслуживающего персонала
- **Сервисные компании** — планирование визитов техников
- **Сборщики мусора** — оптимизация маршрутов

**Используемые алгоритмы:**
- Метод ветвей и границ (для небольших n)
- Локальный поиск с 2-opt, 3-opt эвристиками
- Генетические алгоритмы

## 4. **Биоинформатика: Сборка генома**

**Проблема:** Восстановление последовательности ДНК из коротких фрагментов (ридов).

**Пример:**
- **Медицинские исследования** — расшифровка геномов
- **Фармацевтические компании** — поиск мишеней для лекарств

**Используемые алгоритмы:**
- Жадные алгоритмы сборки
- Графовые эвристики (граф де Брейна)
- Вероятностные методы

## 5. **Машинное обучение: Обучение нейронных сетей**

**Проблема:** Настройка миллионов параметров нейронной сети для минимизации функции потерь.

**Пример:**
- **GPT, ChatGPT** — обучение языковых моделей
- **Компьютерное зрение** — распознавание изображений
- **Рекомендательные системы** — Netflix, YouTube

**Используемые алгоритмы:**
- Градиентный спуск (эвристика 1-го порядка)
- Adam, RMSprop (адаптивные эвристики)
- Используются именно потому, что точное решение невозможно

---

## **Почему именно эвристики?**

1. **NP-трудность** — точные решения требуют экспоненциального времени
2. **Большие данные** — реальные задачи имеют огромную размерность  
3. **"Достаточно хорошие" решения** — на практике часто хватает 95% от оптимального
4. **Ограничения по времени** — решения нужны здесь и сейчас

**Ключевой принцип:** "Лучше быстрое хорошее решение, чем медленное идеальное" — это философия большинства реальных оптимизационных систем!
